    ...
    try {
        if (m.mentionedJid.includes(this.user.jid) && m.isGroup) {
            await this.send2Button(m.chat,
                isBanned ? 'zifabotz tidak aktif' : banned ? 'kamu dibanned' : '*APAAN TOD NGETAG",ADA BANSOS KAH🗿?*',
                '©putrabotz',
...
╰═══════════════════════`.trim(), '©putrabotz', 'PAYMENT', '.payment', 'MENU', '.menu', m)
    ...
    if (new Date() * 1 - setting.status > 1000) {
        ...
        await this.setStatus(`time ${uptime} | 🖥️Mode: ${global.opts['self'] ? 'Private' : setting.groupOnly ? 'Hanya Grup' : 'Publik'} |Putrabotz by.Putra`).catch(_ => _)
